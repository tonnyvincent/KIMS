...
<head>
  ...
  sidebarItems.forEach(function(item) {
    item.addEventListener("click", function() {
      ...
      // Append the selected text as a parameter in the URL's hash fragment
      const currentUrl = new URL(window.location.href);
      currentUrl.hash = `#selectedText=${clickedText}`;
      window.history.pushState({}, '', currentUrl);
    });
  });

  // Retrieve and set the selected text from the URL's hash fragment
  const hashParams = new URLSearchParams(window.location.hash.slice(1));
  const storedText = hashParams.get('selectedText');
  if (storedText) {
